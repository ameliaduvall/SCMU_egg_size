...
# Introduction
This document includes code for likelihood ratio tests to test inclusion of random effects, model selection, and model diagnostics for a linear-mixed model of Scripps's Murrelet *Synthliboramphus scrippsi* egg size as a function of egg order and oceanographic indices at Santa Barbara Island, Channel Islands National Park, USA from 2009-2017.  
\
...
SCMUdf <- left_join(egg, covars, by = "Year") %>% # join by year
  ...
  dplyr::select(Year, Observer, Plot, Size, EggOrder, ANCHL, BEUTI, NPGO, ONI, PDO, SST) 
```
...
# 7 potential predictors (includes EggOrder)
...
```{r global}
## run global model
global_mod <- lmer(Size ~ EggOrder + ANCHL + BEUTI + SST + (1 | Plot), 
...
cov_names[use_set[top6[1],]] # EggOrder, BEUTI, NPGO
cov_names[use_set[top6[2],]] # EggOrder, NPGO, ONI
cov_names[use_set[top6[3],]] # EggOrder, ANCHL, BEUTI, SST (>2 AIC)
cov_names[use_set[top6[4],]] # EggOrder, PDO, SST (>2 AIC)
cov_names[use_set[top6[5],]] # EggOrder, PDO (>2 AIC)
cov_names[use_set[top6[6],]] # EggOrder, BEUTI, SST (>2 AIC)

## run top models
topmod1 <- lmer(Size ~ EggOrder + BEUTI + NPGO + (1 | Plot), 
                data = SCMUdf, REML = TRUE)
topmod2 <- lmer(Size ~ EggOrder + NPGO + ONI + (1 | Plot), 
                data = SCMUdf, REML = TRUE)
topmod3 <- lmer(Size ~ EggOrder + ANCHL + BEUTI + SST + (1 | Plot), 
                data = SCMUdf, REML = TRUE)
topmod4 <- lmer(Size ~ EggOrder + PDO + SST + (1 | Plot), 
                data = SCMUdf, REML = TRUE)
topmod5 <- lmer(Size ~ EggOrder + PDO + (1 | Plot), 
                data = SCMUdf, REML = TRUE)
topmod6 <-lmer(Size ~ EggOrder + BEUTI + SST + (1 | Plot), 
               data = SCMUdf, REML = TRUE)
