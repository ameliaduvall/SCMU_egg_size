...
## create "full" dataset from 2009-2017 for values in year t
ANCHL_full <- ANCHLraw %>%
  slice(-1) %>%
  mutate(time = ymd_hms(time)) %>%
  mutate(Year = year(time)) %>%
...
## create "full_lag" dataset from 2008-2016 for values in year t-1
...
Summary: The NPGO is calculated from an Empirical Orthogonal Function analysis of sea-surface height in the Northeast Pacific. The NPGO is the second dominant mode. We tested this variable under all 4 scenarios.
```{r NPGO}
...
Summary: The PDO is calculated from an Empirical Orthogonal analysis of sea surface temperature anomalies in the North Pacific. The PDO is the first dominant mode. We tested this variable under all 4 scenarios.
```{r PDO}
...
Summary: The ONI is the 3 month running mean of sea surface temperature anomalies in the Nino 3.4 region. We tested this variable under all 4 scenarios.
```{r ONI1}
...
# Plot of each covariate over time
```{r plot}
covarsplot <- covars %>%
