...
# Introduction
This document details steps taken to compile covariate data for a linear mixed model on Scripps's Murrelet (*Synthliboramphus scrippsi*) egg size at Santa Barbara Island within Channel Islands National Park from 2009-2017.

We decided to pull relevant covariates from January-June only because murrelets begin to arrive at the colony in January and clutch initiation extends from March to June (Murray et al. 1985). 
...
```{r sst}
SSTraw <- read.csv(here("data", "covariates", "cciea_OC_SST3_91cf_d165_213f-46025.csv"))
...
SST <- SSTraw %>%
  ...
  filter(Month == 1 | Month == 2 | Month == 3 |Month == 4 | Month == 5 | Month == 6) %>% ## fix all months!!!
  group_by(Year) %>%
...
Buoy 46215  
This buoy was established in 2013. 
NOAA buoy station 46251: https://www.ndbc.noaa.gov/station_page.php?station=46251
Historical data here: https://www.ndbc.noaa.gov/station_history.php?station=46251
...
```{r anchovya}
ANCHAraw <- read.csv(here("data", "covariates", "cciea_EI_FBC_09ec_5398_9325.csv"))
...
Note: No data available for 2017 (check why)  
```{r anchovyl}
# ANCHLraw  <- read.csv(here("data", "covariates", "cciea_EI_FBS_a29e_1fd0_409d.csv"), na.strings = "NaN")
...
```{r krill}
KRILLraw <- read.csv(here("data", "covariates", "cciea_EI_FBC_3c3a_8819_d95c.csv"))
...
```{r PDO}
PDOraw <- read.csv(here("data", "covariates", "cciea_OC_PDO_712b_5843_9069.csv"))
...
```{r ONI}
ONIraw <- read.csv(here("data", "covariates", "cciea_OC_ONI_712b_5843_9069.csv"))
...
```{r NPGO}
NPGOraw <- read.csv(here("data", "covariates", "cciea_OC_NPGO_712b_5843_9069.csv"))
...
```{r CUTI}
CUTIraw <- read.csv(here("data", "covariates", "cciea_OC_CUTI_784c_ef9f_af6f.csv"))
...
```{r bakun}
BAKUNraw <- read.csv(here("data", "covariates", "bakun.csv"), na.strings = "-9999")
...
```{r BEUTI}
BEUTIraw <- read.csv(here("data", "covariates", "cciea_OC_BEUTI_784c_ef9f_af6f.csv"))
...
predictors <- as.data.frame(matrix(c(FALSE, TRUE), 2, 6)) # 6 potential predictors
...
ii <- which(full_set$ANCHL + full_set$NPGO == 2 |
              ...
              full_set$PDO + full_set$SST == 2 ) # 45 models

## create reduced set of models and convert to a matrix for easier indexing
use_set <- as.matrix(full_set[-ii,]) # 19 models

## number of models in set
(n_mods <- nrow(use_set)) # 19 models out of potential 64
```
