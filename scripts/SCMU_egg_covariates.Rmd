...
We selected +/- 0.75 as our cutoff for correlation (Note: Check with Sarah). The following covariates should not be in the same model:
1. NPGO and SST
2. NPGO and PDO
3. ANCHL and PDO
4. PDO and SST
...
ii <- which(full_set$SST + full_set$NPGO == 2 |
              ...
              full_set$SST + full_set$PDO == 2) # 36 models

## create reduced set of models and convert to a matrix for easier indexing
use_set <- as.matrix(full_set[-ii,]) # 28 models

## number of models in set
(n_mods <- nrow(use_set)) # 28 models out of potential 64
```
