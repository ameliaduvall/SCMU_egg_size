...
At first, we decided to pull relevant covariates from January-June only because murrelets begin to arrive at the colony in January and clutch initiation extends from March to June (Murray et al. 1985). Then, we tested lagged and non-lagged covariates (see SCMU_lagged_covariates.Rmd). 
...
For ANCHL, we used data from the full year with no lag (yearly sampling rate). 
```{r anchovyl}
# ANCHLraw  <- read_csv(here("data", "covariates", "cciea_EI_FBS_a29e_1fd0_409d.csv"), na.strings = "NaN")
...
For NPGO, we used data from the entire year (12 month) with a one-year lag. 
```{r NPGO}
...
ii <- which(full_set$ANCHL + full_set$PDO == 2 |
              full_set$ANCHL + full_set$SST == 2 |
              full_set$ANCHL + full_set$NPGO == 2 |
              full_set$BEUTI + full_set$PDO == 2 |
              full_set$NPGO + full_set$PDO == 2 |
              full_set$PDO + full_set$SST == 2) # 42 models

## create reduced set of models and convert to a matrix for easier indexing
use_set <- as.matrix(full_set[-ii,]) # 22 models

## number of models in set
(n_mods <- nrow(use_set)) # 22 models out of potential 64
```
