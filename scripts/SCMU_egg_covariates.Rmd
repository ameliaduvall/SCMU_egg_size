...
Try pulling SST here instead to avoid gaps?
...
We selected +/- 0.65 as our cutoff for correlation (Note: Check with Sarah). The following covariates should not be in the same model:
1. NPGO and SST
2. NPGO and PDO
3. BEUTI and PDO
4. ANCHL and SST
...
Note: Why aren't all of the correlations shown in the plot?  
...
ii <- which(full_set$SST + full_set$NPGO == 2 |
              ...
              full_set$ONI + full_set$PDO == 2) # 49 models

## create reduced set of models and convert to a matrix for easier indexing
use_set <- as.matrix(full_set[-ii,]) # 15 models

## number of models in set
(n_mods <- nrow(use_set)) # 15 models out of potential 64
```
