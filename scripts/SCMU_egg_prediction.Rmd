...
quantile.low <- as.data.frame(apply(pv.BEUTI,2,function(x)quantile(x,probs= 0.025)))
quantile.high <- as.data.frame(apply(pv.BEUTI,2,function(x)quantile(x,probs= 0.975)))
BEUTI.pred <- cbind(y1, quantile.low, quantile.high, SCMU.BEUTI$BEUTI) %>%
...
all <- p1 + p2 + p3
all
ggsave(here("results", "pred_plots.pdf"))
```
