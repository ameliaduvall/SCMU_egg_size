...
df <- left_join(egg, covars, by = "Year") %>%
  ...
  select(Year, Observer, Plot, Size, ANCHL, BEUTI, NPGO, ONI, PDO, SST)
```
...
## add column names 
cov_names <- colnames(predictors) <- colnames(df)[5:10] # these 6 columns are our covariates
...
(n_mods <- nrow(use_set)) # 28 models out of a potential 64 (2^6)
...
mp <- as.data.frame(use_set) 
  # mutate(Model_No = 1:28) %>%
  # pivot_longer(cols = 1:6, names_to = "covariate", values_to = "Included") %>%
  # group_by(Model_No) %>%
  # summarise(Test = 
  #             
  #             
  #             ifelse(Included == TRUE, paste(covariate, sep = ", "), ""))
...
for (i in 1:length(mp)) {
   mp[[i]] <- str_replace(mp[[i]], "FALSE", 0)
}


test <- mp %>%
  mutate(test = ifelse(ANCHL:SST != FALSE, covariate, ""))
...
# 
# mpfe <- mp %>%
#   mutate(FEs = paste(ANCHL, BEUTI, NPGO, ONI, PDO, SST, sep = " "))

usmr <- mpfe %>%
  mutate(k = as.vector(rowSums(use_set) + 2)) %>% # 2 RE's - check this
  mutate(modelno = 1:28) 
