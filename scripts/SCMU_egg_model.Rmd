...
ii <- which(full_set$ANCHL + full_set$PDO == 2 |
              ...
              full_set$PDO + full_set$SST == 2) # 84 models

## create reduced set of models and convert to a matrix for easier indexing
use_set <- as.matrix(full_set[-ii,]) # 44 models

## number of models in set
(n_mods <- nrow(use_set)) # 44 models out of potential 128
...
## New Top Model
```{r ntm}
topmod <- lmer(Size ~ EggOrder + BEUTI + NPGO + ONI + SST + (1 | Plot), data = SCMUdf, REML = TRUE)
summary(topmod)  
...
plot(SCMU.BEUTI$BEUTI[1:100],y = apply(pv,2,mean)[1:100],xlab = "BEUTI",ylab = "Egg Size",type = "l",ylim = c(1500,2500))
lines(SCMU.BEUTI$BEUTI[1:100],y = apply(pv,2,function(x)quantile(x,probs= 0.025))[1:100],lty=2)
lines(SCMU.BEUTI$BEUTI[1:100],y = apply(pv,2,function(x)quantile(x,probs= 0.975))[1:100],lty=2)
