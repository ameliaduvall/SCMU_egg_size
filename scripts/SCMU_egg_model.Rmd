...
###SJC ADDED 
#SCMUdf <- SCMUdf[-c(which(is.na(SCMUdf$Observer==TRUE))),]
```
...
ii <- which(full_set$ANCHL + full_set$PDO == 2 |
              ...
              full_set$NPGO + full_set$PDO == 2) # 92 models

## create reduced set of models and convert to a matrix for easier indexing
use_set <- as.matrix(full_set[-ii,]) # 36 models

## number of models in set
(n_mods <- nrow(use_set)) # 36 models out of potential 128
...
## New Top Model
```{r ntm}
topmod <- lmer(Size ~ EggOrder + BEUTI + NPGO + (1 | Plot), data = SCMUdf, REML = TRUE)
summary(topmod)  
...
```{r d1}
## extract predicted values and plot
preds <- predict(topmod)
ggplot() + 
  geom_histogram(mapping = aes(preds), bins = 20) + # set bins 
  theme_minimal()
```
...
coef(topmod) # this include fixed and random effects
ranef_pl <- ranef(topmod)$Plot # plot random effect only
...
#SJC ADDED 
#based on a strict alpha = 0.05, we would keep plot (p ~ 0.004) but eliminate observer (p ~ 0.09). We can make a decision either way though... 

exactRLRT(bm_obs1, bm1, bm_plot)

exactRLRT(bm_plot, bm1, bm_obs1)
...
##SJC added - here we are doing model selection and calculating Akaike weights 
##model1
# bm1 <- lmer(Size ~ EggOrder + BEUTI + NPGO + ONI + (1 | Observer) + (1 | Plot), data = SCMUdf, REML = TRUE)
# ##model2
# bm2 <- lmer(Size ~ EggOrder + ANCHL + BEUTI + ONI + (1 | Observer) + (1 | Plot), data = SCMUdf, REML = TRUE)
...
# bm3 <- lmer(Size ~ EggOrder + ANCHL + BEUTI + SST + (1 | Observer) + (1 | Plot), data = SCMUdf, REML = TRUE)
# 
# AIC.tab <- matrix(NA,nrow = 3, ncol = 3)
# AIC.tab[1,1] <- extractAIC(bm1)[2]
# AIC.tab[2,1] <- extractAIC(bm2)[2]
# AIC.tab[3,1] <- extractAIC(bm3)[2]
# AIC.tab[,2] <- AIC.tab[,1] - min(AIC.tab[,1])
# AIC.tab[,3] <- exp(-0.5*AIC.tab[,2])/(sum(exp(-0.5*AIC.tab[,2])))
...
AIC.tab <- matrix(NA,nrow = 2, ncol = 3)
AIC.tab[1,1] <- extractAIC(topmod)[2]
AIC.tab[2,1] <- extractAIC(topmod2)[2]
AIC.tab[,2] <- AIC.tab[,1] - min(AIC.tab[,1])
AIC.tab[,3] <- exp(-0.5*AIC.tab[,2])/(sum(exp(-0.5*AIC.tab[,2])))
...
plot(SCMU.SST$SST[1:100],y = apply(pv.SST,2,mean)[1:100],xlab = "SST",ylab = "Egg Size",type = "l",ylim = c(1500,2500))
lines(SCMU.SST$SST[1:100],y = apply(pv.SST,2,function(x)quantile(x,probs= 0.025))[1:100],lty=2)x
lines(SCMU.SST$SST[1:100],y = apply(pv.SST,2,function(x)quantile(x,probs= 0.975))[1:100],lty=2)
