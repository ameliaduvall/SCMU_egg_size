...
topmod5 <- lmer(Size ~ EggOrder + NPGO + (1 | Plot), data = SCMUdf, REML = TRUE)
...
## Top Model 2
```{r topmod2}
topmod2 <- lmer(Size ~ EggOrder + BEUTI + NPGO + ONI + (1 | Plot), data = SCMUdf, REML = TRUE)
summary(topmod2)
```
...
coef(bm_plot) # this include fixed and random effects
ranef_pl <- ranef(bm_plot)$Plot # plot random effect only
