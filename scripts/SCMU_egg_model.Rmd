...
#TODO
- Look into another source for SST temperature due to 2011 gaps in buoy data
- Shift months for NPGO? Dec-March see Chenillat et al. 2012
- Check covariate correlations with new NPGO values
- Calculate confidence intervals
- Prediction…. In a year with good anchovies/bad anchovies, etc
- Diagnostics: ACF plot 
...
## join covariate data with egg data by year without SST
df2 <- left_join(egg, covars, by = "Year") %>%
  filter(TrueOrder == TRUE) %>% # egg order known only
  select(Year, Observer, Plot, Size, EggOrder, ANCHL, BEUTI, NPGO, ONI, PDO)

## join covariate data with egg data by year without 2011 data
df3 <- left_join(egg, covars, by = "Year") %>%
  filter(TrueOrder == TRUE) %>% # egg order known only
  select(Year, Observer, Plot, Size, EggOrder, ANCHL, BEUTI, NPGO, ONI, PDO, SST) %>%
  filter(Year != 2011)
```
...
## here's another way to look at the model results
sjPlot::tab_model(nm)
...
## another way to look at model output 
...
bestm <- allm %>%
  filter(deltaAIC <= 2) 
```
There are two competitive models. 

# Fit Candidate Models without SST
```{r mods2}
## create data frame specifying predictors to include
predictors <- as.data.frame(matrix(c(FALSE, TRUE), 2, 6)) # 6 potential predictors (includes EggOrder)

## add column names
cov_names <- colnames(predictors) <- colnames(df2[,5:10])

## create set of all possible combinations
full_set <- expand.grid(predictors) # 64 combinations

## select models with correlated predictors
ii <- which(  full_set$NPGO + full_set$PDO == 2 |
              full_set$BEUTI + full_set$PDO == 2 |
              full_set$ANCHL + full_set$ONI == 2 |
              full_set$ANCHL + full_set$PDO == 2 |
              full_set$ONI + full_set$PDO == 2) # 38 models

## create reduced set of models and convert to a matrix for easier indexing
use_set <- as.matrix(full_set[-ii,]) # 26 models

## number of models in set
(n_mods <- nrow(use_set)) # 26 models out of potential 64
...
for(i in 1:n_mods) {
  ...
  mod_fit <- lmer(as.formula(fmla), data = df2)
  mod_res[i,"AIC"] <- AIC(mod_fit)
  mod_res[i,"BIC"] <- BIC(mod_fit) 
}
...
mpfe <- mp %>%
  mutate(FEs = paste(EggOrder, ANCHL, BEUTI, NPGO, ONI, PDO, sep = " "))
...
# Fit Candidate Models without 2011 data 
This is the year with gappy SST. We fit the models without 2011 data so that we could include SST as a predictor and see if it made any of the top models. 
```{r mods3}
## create data frame specifying predictors to include
predictors <- as.data.frame(matrix(c(FALSE, TRUE), 2, 7)) # 7 potential predictors (includes EggOrder)

## add column names
cov_names <- colnames(predictors) <- colnames(df3[,5:11])
...
for(i in 1:n_mods) {
  ...
  mod_fit <- lmer(as.formula(fmla), data = df3)
  mod_res[i,"AIC"] <- AIC(mod_fit)
  mod_res[i,"BIC"] <- BIC(mod_fit) 
} #isSingular error
...
bestm <- allm %>%
  filter(deltaAIC <= 2) 
```
There may be issues with model fits (boundary (singular) fit: see ?isSingular) but SST was still not included in the top competitive models. 

# Top Model
Here we look at the top competitive model, but there is a competitive top model also. 
```{r bestmod}
##run model
...
##look at competitive model 
bm2 <- lmer(Size ~ EggOrder + ONI + BEUTI + NPGO + (1 | Observer) + (1 | Plot), data = df)
