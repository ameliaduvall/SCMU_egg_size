...
cov_names[use_set[28,]] # Egg order, BEUTI, SST (>2 AIC)
cov_names[use_set[8,]] # Egg order, ANCHL, BEUTI (>2 AIC)
...
topmod5 <- lmer(Size ~ EggOrder + BEUTI + SST + (1 | Plot), data = SCMUdf, REML = TRUE)
topmod6 <-lmer(Size ~ EggOrder + ANCHL + BEUTI + (1 | Plot), data = SCMUdf, REML = TRUE)
...
=======
>>>>>>> 645e8459c6442f40b202db6ba5d0adc5a6678bbb
# Prediction
```{r predict}
topmod1 <- lmer(Size ~ EggOrder + BEUTI + NPGO + (1 | Plot), data = SCMUdf, REML = TRUE)
topmod2 <- lmer(Size ~ EggOrder + BEUTI + ONI + SST + (1 | Plot), data = SCMUdf, REML = TRUE)
